...
if __name__ == '__main__':
    # 参数说明：
    # p=0.5 - 有50%的概率应用MixStyle
    # alpha=0.1 - Beta分布的参数
    # eps=1e-6 - 数值稳定性参数
    # mix='random' - 使用随机混合方式
    block = MixStyle(p=0.5, alpha=0.1, eps=1e-6, mix='random').to('cuda')

    # 假设输入是batch_size=8, channels=3, height=32, width=32的特征图
    input = torch.rand(8, 3, 32, 32).to('cuda')
